...
const Contact: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="container mx-auto px-6 py-16 -mt-24 relative z-10">
        {/* Glassmorphism Container */}
        <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl overflow-hidden flex flex-col lg:flex-row mb-24 border border-white/50 relative">
           {/* Glow Effect */}
           <div className="absolute top-0 right-0 w-96 h-96 bg-cyan-500/10 rounded-full blur-3xl pointer-events-none"></div>

          {/* Left Panel: Info & Map */}
          <div className="lg:w-5/12 bg-slate-50/80 p-8 flex flex-col gap-8 border-r border-slate-100">
            <div>
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-start gap-4 group">
                  ...
                  <div>
                    ...
                    <p className="text-slate-600">comercial@bodeinter.com</p>
                  </div>
                </div>
                <div className="flex items-start gap-4 group">
                  <div className="w-12 h-12 rounded-full bg-white text-blue-900 shadow-sm flex items-center justify-center flex-shrink-0 group-hover:bg-blue-900 group-hover:text-white transition-colors border border-slate-100">
            ...
            <div className="flex-grow min-h-[300px] rounded-2xl overflow-hidden border-4 border-white shadow-md">
               <Map />
            </div>
          </div>

          {/* Right Panel: Form */}
          <div className="lg:w-7/12 p-8 lg:p-12 bg-white/60 relative">
            <div className="mb-10 relative z-10">
              <span className="text-cyan-600 font-bold tracking-widest uppercase text-xs">Estamos para ayudarte</span>
              <h2 className="text-3xl font-bold text-slate-900 mt-2">Envíenos un mensaje</h2>
        ...
        <div className="max-w-3xl mx-auto py-12">
          <div className="text-center mb-12">
            ...
            <p className="text-slate-500 mt-4">Respuestas a las dudas más comunes de nuestros clientes.</p>
          </div>
          ...
          <div className="space-y-4">
            {faqs.map((faq, idx) => (
              <div key={idx} className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md transition-shadow group">
                ...
                >
                  <span className="font-bold text-slate-800 group-hover:text-blue-900 transition-colors">{faq.q}</span>
                  <span className={`transform transition-transform duration-300 text-cyan-500 ${openFaq === idx ? 'rotate-180' : ''}`}>
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"/></svg>
                  </span>
                </button>
                <div 
                  className={`px-6 overflow-hidden transition-all duration-300 ease-in-out ${
                    openFaq === idx ? 'max-h-40 py-4 border-t border-slate-100' : 'max-h-0'
                  }`}
                >
